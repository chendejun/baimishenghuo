...
//                }
    ...
    public function getCompAction(){
        ...
        $data=array();
        $tagList = $adv->getMacCompList("bind_status=1 and comp_pid={$this->uinfo['comp_id']}".$where);
		if(!empty($tagList)){
    ...
    public function getCompListAction(){
       ...
       $tagList = $adv->getCompList("comp_type=1  and comp_pid=".$uinfo['comp_id'].$where);
    ...
    public function detailAction(){
        ...
        if($crowd_info['label_info']){
            ...
            $tag['type']=array('MAC地址包','IMEI号码包','手机号码包');
