...
//                $account['total_disburse']+=$v['total_disburse'];
            ...
            $portrait['top']=0;
            $portrait['app']=0;
            $data_pt['comp_id']=$this->uinfo['comp_id'];
    ...
    public function remainCashAction(){
    	$p = $this->getRequest()->getParam('comp_id');
    	$p = explode(',', $p);
    	$data = array();
    	foreach ($p as $v) {
    		$v = (int)$v;
    		$cash = array('cash_1' => array('amount' => '0.00' , 'usable_amount' => '0.00') , 'cash_2' => array('amount' => '0.00' , 'usable_amount' => '0.00'));
    		$result = BmDspApi::request('fund/get', array('comp_id' => $v) ,'GET');
	    	if($result['api_code'] == 0){
	    		foreach ($result['data'] as $key => $val) {
	    			if($val['account_type'] == 1){
	    				$cash['cash_2']['amount'] = Helper::transAmount($val['amount'] , 1);
	    				$cash['cash_2']['usable_amount'] = Helper::transAmount($val['usable_amount'] , 1);
	    			}elseif($val['account_type'] == 2){
	    				$cash['cash_1']['usable_amount'] = Helper::transAmount($val['usable_amount'] , 1);
	    				$cash['cash_1']['amount'] = Helper::transAmount($val['amount'] , 1);
	    			}
	    		}
	    	}
	    	$data[$v] = $cash;
    	}
    	Helper::outJson(array('state'=>1,'msg'=>'ok','data'=>$data));
    }
}