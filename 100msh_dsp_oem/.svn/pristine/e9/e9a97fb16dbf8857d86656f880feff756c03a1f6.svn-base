...
class U extends Model{
	...
	public function login($user_name , $pwd){
		...
		$comp_info = $this->db->dsp->table('dsp_company')->field('comp_id,comp_pid,sc_pro,sc_pid,comp_name,mobile_phone,agent_level')->where("comp_id='{$row['comp_id']}' AND status=1")->find();
		if(empty($comp_info)) return '-1';
	...
	public function getlist($step , $limit , $staff_name , $user_group , $is_pages = false){
		$where = "";
		if($staff_name !== '') $where = " AND b.staff_name LIKE \"%{$staff_name}%\"";
		if($user_group > 1){
	 ...
	 * @return [type]          [description]
	 */
	public function compInfo($comp_id){
		return $this->db->dsp->table('dsp_company')->where("comp_id={$comp_id}")->find();
	}
}