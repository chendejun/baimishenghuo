...
Class Delivery extends  Model{
	...
	public function getDliveryCount($where){
       if($where){
           ...
           $oneres = $this->db->dsp->query("SELECT count(*) as count  FROM dsp_delivery WHERE delivery_status=0 ");
       }
        if($where){
            $twores = $this->db->dsp->query("SELECT count(*) as count  FROM dsp_delivery WHERE delivery_status=1 AND  {$where}");
        }else{
            $twores = $this->db->dsp->query("SELECT count(*) as count  FROM dsp_delivery WHERE delivery_status=1");
        }
        if($where){
            $threeres = $this->db->dsp->query("SELECT count(*) as count  FROM dsp_delivery WHERE delivery_status=3  AND {$where}");
        }else{
            $threeres = $this->db->dsp->query("SELECT count(*) as count  FROM dsp_delivery WHERE delivery_status=3");
        }
        if($where){
            ...
            $compingres = $this->db->dsp->query("SELECT comp_id,count(*) AS count  FROM dsp_delivery WHERE  delivery_status=1  GROUP BY comp_id");
        }
	    ...
	    if($where){
            ...
            $comphaveres = $this->db->dsp->query("SELECT comp_id,count(*) AS count  FROM dsp_delivery  GROUP BY comp_id");
        }
...
//根据广告主id获取余额
public function getAccountInfoByCompId($comp_id){
$res= $this->db->account->query("SELECT SUM(usable_amount) AS usable_amount  FROM dsp_account  WHERE comp_id=".$comp_id);
$res_info = $this->db->dsp->query("SELECT comp_name FROM dsp_company WHERE comp_id=".$comp_id);
$data['usable_amount'] = $res[0]['usable_amount'];
$data['comp_name'] = $res_info[0]['comp_name'];
return $data;
}


//广告主总数
public function getallcomp($where){
    if($where){
        $res = $this->db->dsp->query("SELECT count(*) AS count  FROM dsp_company  WHERE ".$where);
        return $res[0]['count'];
    }else{
        $res = $this->db->dsp->query("SELECT count(*) AS count  FROM dsp_company ");
        return $res[0]['count'];
    }
