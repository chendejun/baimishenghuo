...
class Upload implements FileUpload{
    ...
    function Uploads($FormName,$UpFileDir,$UpFileName='',$MaxSize=0,$FileType=array(),$key=null){
        ...
        if(!$this->FileMaxSize($MaxSize)){return -2;}
        if(!$this->get_file_type($FileType)){return -1;}
    ...
    public function base64DecImg($baseData,$Dir,$fileName){
        if(!file_exists($Dir)) $this->createDirectory($Dir,$mode=0777);
        try{
