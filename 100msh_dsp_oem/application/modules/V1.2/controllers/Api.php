...
//        / $users_result = json_decode($users_result , true);
    ...
    //behe定时获取广告主资质审核状态
    public function bhAptTaskAction(){
        Yaf_Dispatcher::getInstance()->disableView();
        set_time_limit(0);
        $bh = Helper::M("Bhadver");
        //1.0执行编辑日期大于等于5天，且同步次数少于3次
        $task=$bh->getBhCompAptTask(1);
        if(empty($task)){
            //2.0执行编辑日期大于等于3天，且同步次数少于3次
            $task=$bh->getBhCompAptTask(2);
            if(empty($task)){
                //3.0执行编辑日期大于等于现在的，且同步次数少于3次
                $task=$bh->getBhCompAptTask(0);
                if(empty($task)){
                    echo "已完成";exit;
                }
            }
        }
        $res=$bh->exeAptTask($task);
        if($res){
            echo "成功";exit;
        }else{
            echo "失败";exit;
