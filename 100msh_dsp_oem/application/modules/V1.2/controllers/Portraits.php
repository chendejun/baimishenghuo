...
class PortraitsController extends BasicController
{
    ...
    /**
     * [getBasicAttributes 得到画像的基本属性]
     * @return [type] [description]
     */
    public function getBasicAttributesAction()
    {
        ...
        $data = $portraits->findPortraits($where);
    ...
    public function getPortraitsInfoAction()
    {
        ...
        foreach ($data as $key => $value) {
            $content = json_decode($value['tab_content'],true);
            unset($content['tdid']);
            $new_data[$key]['content'] = $content;
    ...
    /**
     * [getPortraitsBalanceAction 个体画像余额]
     * @return [type] [description]
     */
    public function getPortraitsBalanceAction()
    {
        ...
        if (!empty($result) && $result['api_code'] == 0 && isset($result['data'])) {
                ...
                Helper::outJson(array('state'=>self::SUCCESS_STATE,'data'=> $one_num));
        }else {
            Helper::outJson(array('state'=>self::FAIL_STATE,'msg'=> 'api返回错误'));
        }
