...
Class Dcrowd extends Model{
    ...
    public function getCrowdlList($where , $start = 0 , $limit=15){
        $result = $this->db->dsp->query("SELECT crowd_id,crowd_name,crowd_taskid,audit_status,cover_users,download_nums,if_download,staff_name,FROM_UNIXTIME(cri.add_time,'%Y-%m-%d %H:%i') as add_time,if(cri.finish_time=0,'~',FROM_UNIXTIME(cri.finish_time,'%Y-%m-%d %H:%i')) as finish_time FROM `dsp_crowd_info` AS cri
         LEFT JOIN dsp_comp_staff AS comp ON comp.staff_id = cri.add_staff_id WHERE {$where}  ORDER BY `add_time` DESC LIMIT $start,$limit");
         return $result;
    }
    /**
     * 根据人群包状态统计数据111
    ...
    public function getCrowdByTaskid($crowd_taskid){
        $result = $this->db->dsp->query("SELECT * FROM `dsp_crowd_info`  WHERE `crowd_taskid`='{$crowd_taskid}'");
        if(!empty($result)){
            return $result[0];
        }
        return array();
    }
    public function dlsCrowdN($comp_pid,$crowdName){
        $result = $this->db->dsp->query("SELECT crowd_id FROM `dsp_crowd_info`  WHERE `comp_pid`='{$comp_pid}' AND crowd_name='{$crowdName}'");
        if(!empty($result)){
            return $result[0];
        }
        return array();
     ...
     */
    public function getCrowdDwListCount($type="0"){
        $list =  $this->db->dsp->query("SElECT count(crowd_id) as count FROM `dsp_crowd_info` WHERE audit_status = 1 AND crowd_taskid !='' AND direct_type IN ({$type})" );
        return $list[0]['count'];
    }
    /**
     * 查询运算中的人群包taskId列表111
     */
    public function getCrowdDwList($start = 0 , $limit,$type="0"){
        $list =  $this->db->dsp->query("SElECT crowd_taskid,crowd_id FROM `dsp_crowd_info` WHERE audit_status = 1 AND crowd_taskid !='' AND direct_type IN ({$type}) LIMIT $start,$limit" );
        return $list;
    }
...
//         var_dump($res);
  ...
  //查询代理商的可投放区域
    ...
    public function getMacList($comp_id){
        $arr=array();
        $list =  $this->db->dsp->query("SElECT mac_addr FROM `dsp_mac_list` WHERE comp_id = {$comp_id} AND mac_status = 1 AND bind_status=1" );
        if(!empty($list)){
            foreach ($list as $k => $v){
                $arr[]=$v['mac_addr'];
            }
