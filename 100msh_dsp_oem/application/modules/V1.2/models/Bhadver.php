...
class Bhadver extends Model
{
    ...
    function exeTask($task){
        ...
        if($type == 1){//新建广告主
            ...
            //b>新增广告主资质图片上传（行业资质图）
            if(!empty($compInfo['industry_img'])){
                $img_data=array();
                $this->config = Yaf_Registry::get('config');
                if(strpos($compInfo['industry_img'], $this->config['config']['ACCESSORY_URL']) !== false){
                    $industry_img=substr($compInfo['industry_img'],strlen($this->config['config']['ACCESSORY_URL']));
                }else{
                    $this->editBhTask($task['id'],array('sync_status' => -1,'sync_msg'=>'行业资质图链接错误'));
                    return false;
                }
                $img_data['files[]']=new curlFile($this->config['config']['ACCESSORY_FOLDER'].$industry_img);
                $img_res_in=json_decode(BhDspApi::upload($img_data),true);
                if(empty($img_res_in)){
                    $this->editBhTask($task['id'],array('sync_status' => -1,'sync_msg'=>'请求行业资质图片上传接口失败'));
                    return false;
                }
                if($img_res_in['Resopnse']['result-code'] != '0'){
                    $this->editBhTask($task['id'],array('sync_status' => 2,'sync_msg'=>'行业资质图片上传失败'));
                    return false;
                }
            }
            //3.0新增广告主资质信息添加
            ...
            $apt_data['aptitudeName']=!empty($img_res_in)?'其他资质':'';
            $apt_data['aptitudeImg']=!empty($img_res_in)?$img_res_in['Resopnse']['files'][0]['path']:'';
            $apt_data['IdCard']='';
            $apt_data['ICP']='';
            $apt_data['dutyPaid']='';
            ...
            //添加资质
            $sql="INSERT INTO bh_aptitude_detail (`comp_id`,`aptitude_id`,`msg`,`status`) VALUES ";
            $apt_list=$this->db->dsp->table("bh_aptitude",false)->where("1")->select();
            foreach ($apt_list as $k=>$v){
                $sql.="({$comp_id},{$v['aptitude_id']},'',0),";
            }
            $sql=trim($sql,',');
            $sql.=" ON DUPLICATE KEY UPDATE msg=VALUES(msg),status=VALUES(status)";
            //echo $sql;
            $this->db->dsp->execute($sql);
            return true;
            ...
            $pre_data['premium']=80;
            $pre_data['premiumType']='account';
            $pre_data['accountId']=$bh_accInfo['bh_account_id'];
            //var_dump($pre_data);
            $pre_res=json_decode(BhDspApi::premium($pre_data),true);
            //var_dump($pre_res);exit;
            if(empty($pre_res)){
    ...
    function getBhCompAptTask($type){
        if($type == 1){
            ...
            $t=date("Y-m-d H:i:s",time()-30*60); //半个小时前
            $num=0;
        }
    ...
    function getAptInfo(){
        $res=array();
        $aptList=$this->db->dsp->table("bh_aptitude",false)->where("1")->select();
        if(!empty($aptList)){
            foreach ($aptList as $k=>$v){
                $res[$v['aptitude_name']]=$v['aptitude_id'];
            }
        }
        return $res;
    }
       ...
       // var_dump($apt_info);
        ...
        if($lh > 0){
            ...
            foreach ($list['pass'] as $v){
                ...
                $aaa=trim($aaa);
                $sql.="({$task['comp_id']},{$apt_info[$aaa]},'{$msg}',1),";
            }
        }
        //2.0
        //echo "2.未通过:<br/>";
        ...
        if($lh > 0){
            ...
            foreach ($list['wait'] as $v){
                ...
                $aaa=trim($aaa);
                $sql.="({$task['comp_id']},{$apt_info[$aaa]},'{$msg}',0),";
            }
        }
        $sql=trim($sql,',');
        $sql.=" ON DUPLICATE KEY UPDATE msg=VALUES(msg),status=VALUES(status)";
        //echo $sql;
        $this->db->dsp->execute($sql);
        $this->editBhComp($task['id'],array('aptitude_times' => $task['aptitude_times']+1));
        return true;
    }
