...
class Notice extends  Model{
    ...
    */
    ...
    public function  getUserNotice($user_id){
        ...
        $data = $this->db->dsp->query("SELECT  dsp_sys_notice.*,dsp_sys_notice_readstate.rid,dsp_sys_notice_readstate.read_status   FROM `dsp_sys_notice_readstate`
                                    LEFT JOIN dsp_sys_notice ON  dsp_sys_notice_readstate.notice_id = dsp_sys_notice.notice_id   WHERE   dsp_sys_notice_readstate.user_id=".$user_id.' order by dsp_sys_notice_readstate.create_time desc ');
        return $data;
    }
    /*
     * 获取未读消息总和
     */
    public function  getCountNotice($user_id){

        $data = $this->db->dsp->table('dsp_sys_notice_readstate')->where(array('user_id'=>$user_id,'read_status'=>1))->count();
        return $data;
    }
    /*
     * 设置已读
